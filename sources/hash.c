...
int hashel_3d(VLst *vlst) {
  ...
  /* alloc hash */
  ht.nmax = (int)(12.71 * vlst->info.np);
  ht.cell = (Cell*)calloc(ht.nmax+2,sizeof(Cell));
  assert(ht.cell);

  ht.hsiz = vlst->info.np;
  ht.hnxt = ht.hsiz;
  ...
  for (k=1; k<=vlst->info.ne; k++) {
    ...
    for (i=0; i<4; i++) {
      ...
      i2 = (i+2) % 4;
      i3 = (i+3) % 4;
      if ( !hcode_3d(vlst->mesh.tetra,&ht,pt->v[i1],pt->v[i2],pt->v[i3],k,i) )  return(0);
      nt++;
    }
  }
  ...
  for (k=1; k<=vlst->info.ne; k++) {
    pt   = &vlst->mesh.tetra[k];
    for (i=0; i<4; i++) {
      if ( !pt->adj[i] )  vlst->mesh.point[pt->v[i]].s = k;
    }
  }
  for (k=1; k<=vlst->info.ne; k++) {
    pt = &vlst->mesh.tetra[k];
...
int hashel_2d(VLst *vlst) {
  ...
  for (k=1; k<=vlst->info.nt; k++) {
    ...
    for (i=0; i<3; i++) {
      i1 = (i+1) % 3;
      i2 = (i+2) % 3;
      if ( !hcode_2d(&vlst->mesh,&ht,pt->v[i1],pt->v[i2],k,i) )  return(0);
      na++;
    }
  }
